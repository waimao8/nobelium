...
import { getDataFromFile, setDataToFile } from '@/lib/local_file_cache'

export async function getAllPosts () {
  const data = await getDataFromFile('post')
  if (data) return data
  ...
  if (rawMetadata?.type !== 'collection_view_page' && rawMetadata?.type !== 'collection_view') {
    ...
    await setDataToFile('posts', posts)
    return posts
  }
}

// 从Block获取文章目录
function getToc (id, block, pageRecordMap) {
  // const parentPageBlock = getBlockParentPage({ parent_id: id }, pageRecordMap)
  // const toc = parentPageBlock ? getPageTableOfContents(parentPageBlock, pageRecordMap) : []
  // const toc = block ? getPageTableOfContents(pageRecordMap) : []
  return []
}
